...
from data.mongo.extracciones_repo import guardar_extraccion, buscar_extraccion_por_hash
from service.auth_service import verificar_credenciales, logout
from service.extraccion_documentos import extraer_documento, ErrorLectura
from service.extraccion_documentos.prellenado import construir_prellenado, LLENO, REVISAR
...
# ══════════════════════════════════════════════════════════════════════════════
# PRELLENADO DESDE DOCUMENTO (formato GESP-FT-14 en Word o PDF)
# ══════════════════════════════════════════════════════════════════════════════

def _aplicar_prellenado(tipo, datos, prellenado):
    """Callback del botón 'Aplicar'. Se ejecuta antes de volver a dibujar la página,
    por eso sí puede escribir las claves de los widgets del formulario."""
    for clave, valor in prellenado["valores"].items():
        st.session_state[clave] = valor
    for nombre, registros in prellenado["listas"].items():
        st.session_state[nombre] = [dict(r) for r in registros]
    id_ext = guardar_extraccion(datos, st.session_state.username, tipo)
    st.session_state[f"_doc_aplicado_{tipo}"] = {
        "archivo": datos["_meta"]["archivo"],
        "campos": len(prellenado["valores"]),
        "guardado": id_ext is not None,
    }


def _panel_carga_documento(tipo):
    """Carga un .docx/.pdf del formato GESP-FT-14, muestra lo extraído y permite aplicarlo."""
    aviso = st.session_state.pop(f"_doc_aplicado_{tipo}", None)
    if aviso:
        st.success(f"✅ Se prellenaron {aviso['campos']} campos y las listas desde **{aviso['archivo']}**. "
                   "Revise cada sección antes de registrar el caso.")
        if not aviso["guardado"]:
            st.warning("⚠️ El formulario se prellenó, pero el JSON extraído no se pudo guardar en la base de datos.")

    with st.expander("📄 Prellenar desde evaluación de riesgo GESP-FT-14 (Word o PDF)", expanded=False):
        st.caption("Cargue el formato diligenciado. Se extraen los datos, se muestran para revisión "
                   "y, al aplicar, se llenan los campos del formulario. El archivo no se almacena; "
                   "solo se guarda el JSON extraído.")
        archivo = st.file_uploader("Archivo .docx o .pdf", type=["docx", "pdf"], key=f"doc_carga_{tipo}")
        if archivo is None:
            return

        contenido = archivo.getvalue()
        sha = hashlib.sha256(contenido).hexdigest()
        _cache_key = f"_doc_extraccion_{tipo}"
        cache = st.session_state.get(_cache_key)
        if not cache or cache["sha"] != sha:
            try:
                with st.spinner("Leyendo el documento..."):
                    datos = extraer_documento(archivo.name, contenido)
                    prellenado = construir_prellenado(datos, tipo)
                ...
                return
            cache = {"sha": sha, "datos": datos, "pre": prellenado,
                     "previo": buscar_extraccion_por_hash(sha)}
            st.session_state[_cache_key] = cache
        datos, prellenado = cache["datos"], cache["pre"]

        if cache["previo"]:
            st.info(f"ℹ️ Este mismo archivo ya se había cargado el {cache['previo'].get('guardado_en', '—')} "
                    f"por **{cache['previo'].get('usuario', '—')}**.")
        for adv in datos["_meta"]["advertencias"]:
            st.warning(f"⚠️ {adv}")

        reporte = pd.DataFrame(prellenado["reporte"])
        listas = prellenado["listas"]
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("Campos", int((reporte["Estado"].isin([LLENO, REVISAR])).sum()) if not reporte.empty else 0)
        c2.metric("Hechos", len(listas["hechos"]))
        c3.metric("Verificaciones", len(listas["verificaciones"]))
        c4.metric("Desplazam.", len(listas["desplazamientos"]))
        c5.metric("Antecedentes", len(listas["antecedentes"]))

        tab_res, tab_json = st.tabs(["📋 Qué se va a llenar", "🧾 JSON extraído"])
        with tab_res:
            solo_con_dato = st.toggle("Ocultar campos sin dato", value=True, key=f"doc_ocultar_{tipo}")
            vista = reporte[reporte["Estado"] != "— Sin dato"] if solo_con_dato and not reporte.empty else reporte
            st.dataframe(vista, use_container_width=True, hide_index=True)
        with tab_json:
            st.json(datos, expanded=False)
            st.download_button("⬇️ Descargar JSON",
                               data=json.dumps(datos, ensure_ascii=False, indent=2, default=str),
                               file_name=f"{archivo.name.rsplit('.', 1)[0]}.json",
                               mime="application/json", key=f"doc_json_{tipo}")

        ocupadas = [n for n in listas if st.session_state.get(n)]
        if ocupadas:
            st.warning("Al aplicar se **reemplazarán** los registros ya agregados en: " + ", ".join(ocupadas) + ".")
        st.button("⬇️ Aplicar al formulario", type="primary", use_container_width=True,
                  key=f"doc_aplicar_{tipo}", on_click=_aplicar_prellenado, args=(tipo, datos, prellenado))
...
def formulario_casos(tipo="individual"):
    ...
    # ── Prellenado desde documento (debe ir ANTES de los widgets del formulario) ──
    if es_individual:
        _panel_carga_documento(tipo)
    ...
    for i, hecho in enumerate(st.session_state.hechos):
        with st.container(border=True):
            if st.session_state.get(_edit_hecho_key) == i:
                ...
                # ── Modo lectura ──────────────────────────────────────────────
                col_tit, col_edit, col_del = st.columns([4, 1, 1])
                with col_tit: st.markdown(f"**Hecho #{i+1} — {hecho['tipo'] or '⚠️ Tipo de Hecho pendiente'}**")
                with col_edit:
    ...
    if registrar:
        ...
        # Registros que llegan del prellenado por documento pueden venir incompletos
        for _i, _h in enumerate(st.session_state.hechos, start=1):
            if not _h.get("tipo") or _h.get("tipo") == "Seleccione...":
                errores.append(f"Hecho de riesgo #{_i}: debe seleccionar el Tipo de Hecho (✏️ Editar)")
        for _i, _d in enumerate(st.session_state.desplazamientos, start=1):
            if not _d.get("motivo") or _d.get("motivo") == "Seleccione...":
                errores.append(f"Desplazamiento #{_i}: debe seleccionar el motivo (✏️ Editar)")
